create or replace table operations.finance.fact_income_statement (
    ...
    ,duplicate_stock_symbol_identifier integer comment "Determines if there are two company_stock_symbol values in the dim_company table. When a company has two stock symbols under the same name, it usually means the company has issued multiple classes of shares, typically to give founders or insiders greater voting control while letting the public invest" 
